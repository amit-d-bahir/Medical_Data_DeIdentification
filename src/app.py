...
def main():
    ...
    if st.button('SUBMIT', use_container_width=True):
        ...
        if "files" in deidentified_data and deidentified_data["files"] != []:
            ...
            for i, file in enumerate(deidentified_data["files"]):
                with open(file, "rb") as f:
                    bytes_data = f.read()
                filename = "deidentified_" + uploaded_files[i].name[:-3] + "txt"
                st.download_button("Download", data=bytes_data, file_name=filename, mime="text/plain")
                st.text(filename)
...
if __name__ == '__main__':
    main()
    # Un-comment this for to start the flask app. Will not work with streamlit
