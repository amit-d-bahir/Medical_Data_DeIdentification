...
def _pdf_ocr(pdf_file_path):
    ...
    now = datetime.now()
    for page in pages:
        filename = "../results/page_" + now.strftime("%d-%m-%Y_%H:%M:%S:%f_") + str(image_counter) + ".jpg"
        page.save(filename, 'JPEG')
        image_counter = image_counter + 1

    txt_file_path = "../results/" + "to_be_deidentified" + now.strftime("%d-%m-%Y_%H:%M:%S:%f") + ".txt"

    with open(txt_file_path, "a") as f:
        for i in range(1, image_counter):
            filename = "../results/page_" + now.strftime("%d-%m-%Y_%H:%M:%S:%f_") + str(i) + ".jpg"
