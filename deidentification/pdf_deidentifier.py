...
def pdf_deidentifier(pdf_file_path):
    ...
    now = datetime.now()
    deid_pdf_path = "../results/" + "deidentified_" + now.strftime("%d-%m-%Y_%H:%M:%S:%f") + ".txt"
...
def pdf_ocr(pdf_file_path):
    ...
    now = datetime.now()
    for page in pages:
        filename = "../results/temp/page_" + now.strftime("%d-%m-%Y_%H:%M:%S:%f_") + str(image_counter) + ".jpg"
        page.save(filename, 'JPEG')
        image_counter = image_counter + 1

    txt_file_path = "../results/temp/" + "to_be_deidentified" + now.strftime("%d-%m-%Y_%H:%M:%S:%f") + ".txt"

    with open(txt_file_path, "a") as f:
        for i in range(1, image_counter):
            filename = "../results/temp/page_" + now.strftime("%d-%m-%Y_%H:%M:%S:%f_") + str(i) + ".jpg"
